...
def _build_engine(provider_name: str, model: str | None):
    ...
    for tool in [BashTool(), FileReadTool(), FileWriteTool(), FileEditTool(), GrepTool(), GlobTool()]:
...
def run(
    ...
    file: Optional[Path] = typer.Option(None, "--file", "-f", help="Read task from file."),
    ...
    engine = _build_engine(provider, model)
